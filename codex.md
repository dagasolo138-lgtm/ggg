...
8. 对具有 `display:flex`、`display:grid` 或 `position:fixed` 的弹层，必须同时设置组件级 `[hidden] { display:none!important; }` 规则。
...
### 2026-06-22 · v0.2.6 · 重命名弹窗组件级隐藏兜底
- 真实根因：`.rename-dialog` 自身声明 `display:grid`，此前只依赖全局 `[hidden]` 规则；当旧 Pages 产物、样式覆盖或移动端 Safari 的 `hidden` 处理出现差异时，弹窗仍可能在首次打开时可见。
- 涉及文件：`src/styles/conversation-actions.css`、`codex.md`。`src/ui/layout.js` 保持初始 `hidden` 与 `aria-hidden="true"`；`src/features/conversations/conversationActions.js` 保持初始化 `closeRenameDialog()`，并且只有内部 `openRenameDialog()` 会移除 `hidden`。
- 修复方式：新增 `.rename-dialog[hidden]{display:none!important}`，以组件选择器强制覆盖 `.rename-dialog{display:grid}`。
- 验证：源码静态检查确认初始 DOM、ARIA、初始化关闭路径和显示入口；GitHub Pages 工作流对 `main` 使用 `npm install` 与 `npm run build`。本次提交后的 Actions / Pages 状态需要以该提交对应的运行结果为准。
- 风险：若用户设备持续使用缓存的旧 CSS，刷新后仍可短暂看到旧行为；新部署完成后需用无缓存刷新或 Safari 关闭后重开验证。

### 2026-06-22 · v0.2.7 · 重命名弹窗修复完成并恢复 Pages 构建
- 真实根因：重命名弹窗自身使用 `.rename-dialog{display:grid}`，仅依赖全局 `[hidden]` 保护不足。组件级 `.rename-dialog[hidden]{display:none!important}` 已加入，强制覆盖显示布局。
- 涉及文件：`src/styles/conversation-actions.css`、`src/ui/layout.js`、`src/features/conversations/conversationActions.js`、`src/styles/app.css`、`src/app/createConversationApp.js`、`index.html`、`.github/workflows/deploy-pages.yml`、`src/features/artifacts/artifactStore.js`、`codex.md`。本次直接改动为组件 CSS、Artifact 配置导出、部署诊断清理与本文记录；其余文件已复查启动与样式加载路径。
- 启动约束：DOM 初始保留 `hidden` 与 `aria-hidden="true"`；初始化主动调用 `closeRenameDialog()`；显示状态只由 `openRenameDialog()` 打开。未发现第二个重命名弹窗 DOM。
- 构建阻塞修复：此前 Pages 的 `npm run build` 因 `artifactView.js` / `artifactWorkspace.js` 导入的 `getArtifactTypeConfig` 未从 `artifactStore.js` 导出而失败。已在 `artifactStore.js` 恢复同类型配置导出；未修改 Artifact、本地对话、API Key 或其他 localStorage 数据。
- 验证结果：GitHub Actions #94 与 #95 均成功完成 `npm install`、`npm run build`、Pages artifact 上传和 Pages 部署；#95 对应提交 `0003f6a`。最终本次 Codex 提交会再次触发同一工作流，需以其结果作为最终 `main` 部署状态。
- 剩余风险：未在实体 iPhone Safari 上执行人工点按回归。若设备仍缓存旧 CSS，需要无缓存刷新或关闭 Safari 后重新打开。